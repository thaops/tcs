...
class LeaveCareateController extends GetxController {
  ...
  Future<void> save_create(BuildContext context) async {
    ...
    try {
      ...
      final response =
          await dioApi.post(ApiEndpoints.careateleave, data: addData);
      print("response: $response");
      if (response.data['statusCode'] != SUCCESS_CODE) {
        Get.snackbar("Thất bại", "${response.data['message']}");
        return;
      }
  ...
  Future<void> fetchLeave() async {
    try {
      ...
      if (response != null) {
        ...
        leaves = data.map((item) => LeaveType.fromJson(item)).toList();
      } else {
